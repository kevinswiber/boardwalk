...
| `https://rels.boardwalk.dev/server`           | root → server, device → server   |
| `https://rels.boardwalk.dev/device`           | server → device sub-entity       |
| `https://rels.boardwalk.dev/peer`             | server → peer server             |
| `https://rels.boardwalk.dev/peer-management`  | root → peer-management           |
| `https://rels.boardwalk.dev/events`           | root → multiplex WS              |
| `https://rels.boardwalk.dev/type`             | device → its metadata            |
| `https://rels.boardwalk.dev/object-stream`    | device → its monitored stream    |
| `https://rels.boardwalk.dev/query`            | search results → live WS         |
| `https://rels.boardwalk.dev/root`             | peer entry → its root            |
| `https://rels.boardwalk.dev/instances`        | meta/type → instances query      |
| `https://rels.boardwalk.dev/metadata`         | type → metadata collection       |
...
`https://rels.boardwalk.dev/query` link pointing at a live WS subscription
for the same query.
...
{
  "title": "state",
  "rel": ["monitor", "https://rels.boardwalk.dev/object-stream"],
