...
- ✅ Graceful shutdown via `Boardwalk::listen_until(addr, signal)`.
...
- ✅ Multi-device observe: `ServerHandle::observe(queries, callback)`
  ...
  device-set changes.
- ✅ Transition-dispatch macro: `boardwalk_core::transitions! { ... }`
  in `Device` impl removes the `Box::pin(async move { match ... })`
  boilerplate.
- ✅ Full proc-macro `#[device]` + `#[transition]` on `boardwalk-macros`
...
- ✅ Peer records persisted on first confirm (alongside device records)
  when `Boardwalk::persist(path)` is enabled.
...
`boardwalk` may be taken. Check before first publish; `boardwalk-rs` is the
fallback per `v1#Q9`.
