//! Proc macros for ergonomic Zetta drivers.
//!
//! ```ignore
//! use zetta::{Device, DeviceConfig, DeviceError};
//! use zetta_macros::device;
//!
...
pub fn device(_attr: TokenStream, input: TokenStream) -> TokenStream {
    ...
    let expanded = quote! {
        ...
        impl #impl_generics ::zetta_core::Device for #self_ty #where_clause {
            fn config(&self, cfg: &mut ::zetta_core::DeviceConfig) {
            ...
            fn transition<'__a>(
                &'__a mut self,
                name: &'__a str,
                _input: ::zetta_core::TransitionInput,
            ) -> ::futures::future::BoxFuture<
                '__a,
                ::std::result::Result<(), ::zetta_core::DeviceError>,
            > {
                ::std::boxed::Box::pin(async move {
                    match name {
                        #(#arms)*
                        other => ::std::result::Result::Err(
                            ::zetta_core::DeviceError::Invalid(
