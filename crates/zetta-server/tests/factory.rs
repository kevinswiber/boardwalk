//! Hubless device registration via Zetta::register_factory.
...
async fn serve(zetta: Zetta) -> SocketAddr {
    let built = zetta.build().unwrap();
...
async fn no_factories_returns_501() {
    let addr = serve(Zetta::new().name("hub")).await;
    let client = reqwest::Client::new();
    let resp = client
        .post(format!("http://{addr}/servers/hub/devices"))
        .header("content-type", "application/x-www-form-urlencoded")
