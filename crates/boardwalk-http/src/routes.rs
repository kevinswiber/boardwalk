...
use boardwalk_core::TransitionInput;
use boardwalk_siren::SIREN_CONTENT_TYPE;
...
pub type DeviceRegistrar = Arc<
    dyn Fn(DeviceRegistration) -> BoxFuture<'static, Result<Uuid, boardwalk_core::DeviceError>>
        + Send
        + Sync,
>;
...
fn siren_response(entity: boardwalk_siren::Entity) -> Response {
...
async fn devices_post(
    ...
    let new_id = match registrar(reg).await {
        Ok(id) => id,
        Err(boardwalk_core::DeviceError::Invalid(msg)) => {
            return (StatusCode::BAD_REQUEST, msg).into_response();
        }
        Err(boardwalk_core::DeviceError::Conflict(msg)) => {
...
async fn device_post(
    ...
    match core.run_transition(&id, &action_name, input).await {
        ...
        Err(boardwalk_core::DeviceError::NotAllowed(_)) => (
            StatusCode::CONFLICT,
...
async fn server_events_stream(
    ...
    let pattern = match boardwalk_events::TopicPattern::parse(&topic) {
        Ok(p) => p,
    ...
    let sub = state
        .core
        .bus
        .subscribe(pattern, boardwalk_events::SubscribeOpts::default());
    let mut rx = sub.rx;
...
async fn meta_type_get(
    ...
    match dev {
        Some(d) => siren_response(
            boardwalk_siren::Entity::new()
                .with_class("type")
                .with_property("type", JsonValue::String(d.type_.clone()))
                .with_link(boardwalk_siren::Link::new(
                    boardwalk_siren::rels::SELF,
...
/// Reuse `boardwalk_tunnel::build_upgrade_response`.
fn boardwalk_tunnel_upgrade_response(headers: &HeaderMap) -> Result<Response, String> {
    let resp = boardwalk_tunnel::build_upgrade_response(headers).map_err(|e| format!("{e}"))?;
...
fn filter_by_ql(
    ...
    let q = match boardwalk_caql::parse(ql) {
        Ok(q) => q,
    ...
    devices
        ...
        .filter(|d| {
            ...
            boardwalk_caql::matches(&q, &target).unwrap_or(false)
        })
        .cloned()
        .collect()
}
