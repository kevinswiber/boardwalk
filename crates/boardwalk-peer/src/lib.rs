...
use axum::Router;
use boardwalk_http::{Core, PeerInitState, PeerSenders};
use boardwalk_registry::{PeerDirection, PeerRecord, PeerStatus, Registry};
use http::Request;
...
pub enum PeerError {
    #[error("tunnel: {0}")]
    Tunnel(#[from] boardwalk_tunnel::TunnelError),
    #[error("io: {0}")]
...
impl PeerClient {
    ...
    async fn attempt_once(&self, connection_id: Uuid) -> Result<(), PeerError> {
        ...
        let ready = boardwalk_tunnel::dial_initiator(
        ...
        let result = hyper::server::conn::http2::Builder::new(TokioExecutor::new())
            ...
            .await
            .map_err(|e| boardwalk_tunnel::TunnelError::Upgrade(format!("h2 serve: {e}")));
...
/// Cloud-side query forwarder. Implements `boardwalk_http::PeerSenders` so
...
async fn drive_acceptor(
    ...
    if resp.status() != http::StatusCode::OK {
        return Err(PeerError::Tunnel(boardwalk_tunnel::TunnelError::Response(
